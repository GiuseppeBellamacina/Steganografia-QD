...
class TestDisplayBackupOptions:
    ...
    def test_display_backup_options_automatic(
        self, mock_uploader, mock_radio, mock_subheader
    ):
    ...
    @patch("streamlit.radio")
    @patch("streamlit.file_uploader")
    def test_display_backup_options_file(
        self, mock_uploader, mock_radio, mock_subheader
    ):
    ...
    def test_display_backup_options_manual(self, mock_radio, mock_subheader):
        ...
        backup_path, use_recent, manual = display_backup_options(
            "test_key", show_manual=True
        )
...
class TestHideDataPages:
    ...
    def test_hide_string_page_no_upload(
        self,
        mock_button,
        mock_input,
        mock_area,
    ...
    @patch("streamlit.text_input")
    @patch("streamlit.button")
    def test_hide_string_page_with_upload_no_message(
        self,
        mock_button,
        mock_input,
        mock_area,
    ...
    @patch("streamlit.text_input")
    @patch("streamlit.button")
    def test_hide_string_page_button_no_image(
        self,
        mock_button,
        mock_input,
        mock_area,
    ...
    @patch("streamlit.file_uploader")
    @patch("streamlit.button")
    def test_hide_image_page_no_upload(
        self, mock_button, mock_uploader, mock_subheader, mock_display
    ):
    ...
    @patch("streamlit.file_uploader")
    @patch("streamlit.button")
    def test_hide_binary_page_no_upload(
        self, mock_button, mock_uploader, mock_subheader, mock_display
    ):
    ...
    def test_hide_string_page_success(self, monkeypatch, tmp_path):
        from src.ui.hide_pages import HideDataPages

        host_path = tmp_path / "host.png"
        img = Image.new("RGB", (100, 100), color="green")
        img.save(host_path, format="PNG")
        ...
        def fake_hide_message(img_obj, message):
            return Image.open(str(host_path))

        monkeypatch.setattr(
            "src.steganografia.hide_message", fake_hide_message, raising=False
        )
        ...
        monkeypatch.setattr(
            "streamlit.file_uploader", lambda *a, **k: MagicMock(name="host", spec=[])
        )
    ...
    def test_hide_image_page_success(self, monkeypatch, tmp_path):
        from src.ui.hide_pages import HideDataPages

        host_path = tmp_path / "host.png"
        secret_path = tmp_path / "secret.png"
        img1 = Image.new("RGB", (100, 100), color="green")
        img1.save(host_path, format="PNG")
        img2 = Image.new("RGB", (100, 100), color="green")
        img2.save(secret_path, format="PNG")
        ...
        def fake_hide_image(img1_obj, img2_obj, lsb, msb, div, backup_file):
            return Image.open(str(host_path)), 1, 8, 0.0, None, None

        monkeypatch.setattr(
            "src.steganografia.hide_image", fake_hide_image, raising=False
        )
        monkeypatch.setattr("src.ui.hide_pages.cleanup_temp_file", lambda p: None)
        mock_create = MagicMock()
        monkeypatch.setattr("src.ui.hide_pages.create_download_button", mock_create)
        ...
        # Streamlit inputs
        monkeypatch.setattr(
            "streamlit.file_uploader", lambda *a, **k: MagicMock(name="file", spec=[])
        )
    ...
    def test_hide_binary_page_success(self, monkeypatch, tmp_path):
        from src.ui.hide_pages import HideDataPages
        ...
        def fake_hide_bin_file(img_obj, secret_path, zip_mode, n, div, backup_file):
            return Image.open(str(host_path)), 1, 0.0, len(b"hello world")

        monkeypatch.setattr(
            "src.steganografia.hide_bin_file", fake_hide_bin_file, raising=False
        )
        monkeypatch.setattr("src.ui.hide_pages.cleanup_temp_file", lambda p: None)
        mock_create = MagicMock()
        monkeypatch.setattr("src.ui.hide_pages.create_download_button", mock_create)
        ...
        monkeypatch.setattr(
            "streamlit.file_uploader", lambda *a, **k: file_mocks.pop(0)
        )
...
class TestRecoverDataPages:
    ...
    def test_recover_string_page_no_upload(
        self, mock_info, mock_button, mock_uploader, mock_subheader, mock_display
    ):
    ...
    @patch("streamlit.button")
    @patch("streamlit.info")
    def test_recover_string_page_button_no_image(
        self,
        mock_info,
        mock_button,
        mock_uploader,
    ...
    @patch("streamlit.file_uploader")
    @patch("streamlit.button")
    def test_recover_image_page_no_upload(
        self, mock_button, mock_uploader, mock_subheader, mock_display
    ):
    ...
    @patch("streamlit.file_uploader")
    @patch("streamlit.button")
    def test_recover_image_page_button_no_image(
        self, mock_button, mock_uploader, mock_subheader, mock_display, mock_warning
    ):
    ...
    @patch("streamlit.file_uploader")
    @patch("streamlit.button")
    def test_recover_binary_page_no_upload(
        self, mock_button, mock_uploader, mock_subheader, mock_display
    ):
    ...
    @patch("streamlit.file_uploader")
    @patch("streamlit.button")
    def test_recover_binary_page_button_no_image(
        self, mock_button, mock_uploader, mock_subheader, mock_display, mock_warning
    ):
    ...
    def test_recover_string_page_success(self, monkeypatch, tmp_path):
        from src.ui.recover_pages import RecoverDataPages
        ...
        monkeypatch.setattr(
            "src.ui.recover_pages.save_uploaded_file", lambda uploaded: str(host_path)
        )
        monkeypatch.setattr(
            "src.steganografia.get_message",
            lambda img_obj: "hello world",
            raising=False,
        )
        mock_create = MagicMock()
    ...
    def test_recover_image_page_success(self, monkeypatch, tmp_path):
        from src.ui.recover_pages import RecoverDataPages
        ...
        def fake_get_image(
            img_obj, output_name, lsb, msb, div, width, height, backup_file_path
        ):
            return Image.open(str(host_path))

        monkeypatch.setattr(
            "src.steganografia.get_image", fake_get_image, raising=False
        )
        monkeypatch.setattr(
            "src.ui.recover_pages.display_backup_options",
    ...
    def test_recover_binary_page_success(self, monkeypatch, tmp_path):
        from src.ui.recover_pages import RecoverDataPages
        ...
        def fake_get_bin_file(
            img_obj, output_name, zip_mode, n, div, size, backup_file_path
        ):
        ...
        monkeypatch.setattr(
            "src.steganografia.get_bin_file", fake_get_bin_file, raising=False
        )
        mock_create = MagicMock()
        monkeypatch.setattr("src.ui.recover_pages.create_download_button", mock_create)
...
class TestResultDisplay:
    ...
    @patch("streamlit.info")
    def test_show_success_message_with_details(
        self, mock_info, mock_expander, mock_success
    ):
    ...
    @patch("streamlit.expander")
    @patch("streamlit.write")
    def test_show_error_message_with_suggestions(
        self, mock_write, mock_expander, mock_error
    ):
        ...
        ResultDisplay.show_error_message(
            "Error", suggestions=["Suggestion 1", "Suggestion 2"]
        )
    ...
    def test_show_download_button(self, mock_download):
        ...
        ResultDisplay.show_download_button(
            b"data", "file.txt", "text/plain", "Download"
        )

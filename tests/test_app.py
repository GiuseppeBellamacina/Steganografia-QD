...
def mock_streamlit():
    ...
    with patch("streamlit.set_page_config"), patch("streamlit.title"), patch(
        "streamlit.markdown"
    ), patch("streamlit.sidebar") as mock_sidebar, patch("streamlit.error"), patch(
        "streamlit.code"
    ):
        yield
...
def mock_modules(mock_streamlit):
    ...
    with patch("app.AppLayout") as mock_layout, patch(
        "app.DynamicInstructions"
    ) as mock_instructions, patch("app.HideDataPages") as mock_hide, patch(
        "app.RecoverDataPages"
        ...
        mock_layout.setup_sidebar = MagicMock(
            return_value=("Nascondere dati", "Stringhe")
        )
...
def test_main_hide_string_mode(mock_modules):
    ...
    mock_modules["instructions"].show_instructions.assert_called_once_with(
        "Nascondere dati", "Stringhe"
    )
...
def test_main_hide_binary_mode(mock_modules):
    ...
    mock_modules["layout"].setup_sidebar.return_value = (
        "Nascondere dati",
        "File binari",
    )
